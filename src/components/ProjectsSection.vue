    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        <template v-for="(project, index) in projectKeys" :key="index">
          <Card
            class="group bg-[var(--color-surface)] border border-[var(--color-border)] transition-all duration-300 hover:shadow-xl hover:scale-105 flex flex-col">
            <CardContent class="p-6 flex flex-col flex-grow">
              <div class="flex items-center gap-3 mb-4">
                <div
                  class="w-12 h-12 bg-[var(--color-primary)]/10 rounded-lg flex items-center justify-center transition-colors duration-300">
                  <component :is="project.icon"
                    class="w-6 h-6 text-[var(--color-primary)] transition-colors duration-300" />
                </div>
                <div class="flex-1">
                  <h3 class="text-lg font-bold text-[var(--color-text)] transition-colors duration-300">{{
                    t(`projects.items.${project.key}.title`) }}</h3>
                </div>
              </div>

              <p
                class="text-[var(--color-text-secondary)] text-sm leading-relaxed transition-colors duration-300 flex-grow">
              ...
              <div class="mt-4 pt-4 border-t border-[var(--color-border)] transition-colors duration-300">
                <div class="flex items-center justify-between">
                  <div
                    class="flex items-center gap-2 text-xs text-[var(--color-text-secondary)] transition-colors duration-300">
                    <component :is="project.icon" class="w-4 h-4" />
                    <span>{{ project.category }}</span>
                  </div>
                  <div class="flex gap-2">
                    <Button v-if="project.hasGithub" variant="ghost" size="icon"
                      class="w-8 h-8 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] hover:bg-[var(--color-primary)]/10 transition-colors duration-300 cursor-pointer"
                      @click="openUrl(project.githubUrl)">
                      <Github class="w-4 h-4" />
                    </Button>
                    <Button v-if="project.hasUrl" variant="ghost" size="icon"
                      class="w-8 h-8 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] hover:bg-[var(--color-primary)]/10 transition-colors duration-300 cursor-pointer"
